...
This runs the agent on a default test query. To test your own query, edit the `test` variable at the bottom of `pilot.py`.
...
# Run a specific test
python eval_report.py 1

# Run multiple specific tests
python eval_report.py 1 3 5
```
...
```
IndexPilot/
  seed_db.py        -- Creates the test database (2 tables, 1.1M rows, no indexes)
  tools.py          -- 6 tools the LLM can call + utility functions for benchmarking
  pilot.py          -- The agent: tool definitions, Groq API integration, agent loop
  eval_report.py    -- Benchmark suite: runs tests, measures speedup, reports results
  indexpilot.db     -- SQLite database (generated by seed_db.py)
```
